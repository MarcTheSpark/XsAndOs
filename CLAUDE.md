...
- **Config files** (plain text, same family): **`Cues/cues.txt`** the cue sheet (carving named beat-windows out of the `.mid` files in `Cues/`; currently `tesCue.mid`, a ~20-beat test cue), **`tempos.txt`** the saved tempo curves, **`midi_map.txt`** the saved MIDI bindings. All three are created/edited by their respective save functions or by hand and auto-loaded by SETUP if present.
...
- **Tempo GUI** (`~tempoGui.()`): opens the curve editor — pick a cue, drag breakpoints (x = beats, y = BPM), set a curve type per segment, add/remove nodes, **audition** (plays the cue), **save all** (writes `tempos.txt`). Edits write straight into `~tempoEnvs[cueName]`, so `~play`/`~next` use them immediately; saving makes them persist.
...
4. **`~playCue.(index)`** — the primitive. Picks the cue's tempo env, maps every note's start/end beat through `~beatToTime`, sorts by absolute seconds, then plays them from a `Routine` on **`SystemClock` in absolute seconds** (no `TempoClock` — tempo is already baked in). Each cue gets its own `Group`. Interrupts whatever is playing first.
...
The synth is `\sawCue`: a saw voice with two envelopes — `ampEnv` (gives the note its computed duration, self-frees) and `killEnv` (a sustained gate that `~stopCue` drops to fade out cleanly without a click).
