...
- **`lib/boardGui.scd`** — the **Xs-and-Os visualization board** (F2). A single compilation unit, `executeFile`'d by SETUP. Defines `~boardGui` (opens a 3×3 grid window) and `~boardEvent.(pitch, vel)` (the hook the engine calls for every note on a `visual board` track).
...
- **Performer commands** (after SETUP): `~play.(id)` takes a cue **name** (`"1"`, `"big finish"`, …) or, failing a name match, a **1-based** cue number (`~play.(1)` is the first cue, matching the score); `~next.()` plays the cue at the cursor and advances; `~stop.()` fades out the current cue, or — **if already stopped** — rewinds the cursor one cue without playing (so the next `~next` replays it).
...
A track mapped `visual board` in `tracks.txt` feeds its notes (still on the cue's tempo-warped timeline, scheduled in the *same* `~playCue` routine) to **`~boardEvent.(pitch, vel, dur)`** in `boardGui.scd` (`dur` is the note's tempo-warped length in seconds). Pitch decodes to a move: **60–68** = Player 1 (X), **72–80** = Player 2 (O), `square = pitch - base` (0–8, row-major); **48** clears the board — **`~boardFadeOut` fades the marks out over `dur`** (the clear note's own length) on AppClock, then resets the state (so the clear note's duration is the fade time; a zero-length clear is instant). A new move cancels any in-flight fade (finishing the clear first, so it lands on a fresh board). **Premature cutoff honors the clear instantly:** the engine tracks `~pendingClears` (clear notes in the playing cue not yet fired), and `~stopCue` — hit by `~stop` *and* by starting another cue — snaps the board to cleared (dur 0) if the interrupted cue had a clear that was still pending **or** mid-fade, so cutting a cue short never leaves a stale or half-faded board. **Velocity 127** marks a winning-line cell (coloured turquoise P1 / maroon P2); the newest move is drawn **bold**. Board state lives in `boardGui.scd` (`~boardState`/`~boardNewest`) and persists between cues — reset relies on the 48-clear note, so a window starting mid-game keeps prior state. The cue routine runs on `SystemClock`, so `~boardEvent` defers all GUI work to AppClock (`.defer`). It is a no-op if the board window isn't open, so audio doesn't depend on it.

### Parameter curves (F3)

Beyond tempo, any `(cue, track, param)` can have its own **value-over-beats** `Env` in **`~paramEnvs`** (keyed by the composite string `"cue\ttrack\tparam"`; track `"*"` = all tracks of the cue). `~paramList` (`\cutoff`, `\pan`, `\amp`) names the editable params — each must be a control the SynthDefs declare (the shared synth contract now includes `cutoff` + `pan`). Control model is **onset sampling**: in `~playCue`, **`~paramArgsAt.(cueName, track, startBeat)`** samples every curve matching that note's `(cue, track)` plus the `"*"` curves and appends `\param, value` to the `Synth` args (unknown control names are harmless). Tempo stays the privileged exception — per-cue, in `~tempoEnvs`, never per-track. Persistence is unified into `tempos.txt`: the line format is now `name | track | param | levels | durations | curves` (tempo written as `name | * | tempo | …`); `~loadTempoEnvs` still reads the legacy 4-field tempo lines (`name | levels | durations | curves`).
